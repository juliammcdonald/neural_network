...
class Linear(Module):
    ...
    def forward(self, A):
        ...
        return np.matmul(self.W.T, A) + self.W0   # Your code (n x b)

    def backward(self, dLdZ):  # dLdZ is (n x b), uses stored self.A
        self.dLdW = np.matmul(self.A, dLdZ.T)       # Your code
        self.dLdW0 = np.sum(dLdZ, 1, keepdims=True) # Your code
        return np.matmul(self.W, dLdZ)              # Your code: return dLdA (m x b)

    def sgd_step(self, lrate):                    # Gradient descent step
        self.W = self.W-lrate*self.dLdW           # Your code (m x n)
        self.W0 = self.W0-lrate*self.dLdW0        # Your code (n x 1)
...
class ReLU(Module):              # Layer activation
    ...
    def backward(self, dLdA):    # Uses stored self.A
        is_pos = self.A > 0
        is_pos = is_pos.astype(np.int)
        return dLdA*is_pos              # Your code: return dLdZ (?, b)


class SoftMax(Module):           # Output activation
    def forward(self, Z):
        self.A = np.exp(Z)
        den_sum = np.sum(self.A, 0, keepdims=True)
        self.A = self.A/den_sum
...
class NLL(Module):       # Loss
    def forward(self, Ypred, Y):
        self.Ypred = Ypred
        self.Y = Y
        return -1*np.sum(self.Y*np.log(self.Ypred))      # Your code
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            t = np.random.randint(D)              # Choose random index
            Xt = X[:, t:t+1] 
            Yt = Y[:, t:t+1]
            Ypred = self.forward(Xt)              # Compute pred output
            cur_loss = self.loss.forward(Ypred, Yt) # Compute loss
            self.backward(self.loss.backward())   # Compute gradients
            self.sgd_step(lrate)                  # Change the weights
            
            self.print_accuracy(it, X, Y, cur_loss)  # Your code
...
######################################################################
#   Data Sets [Written by 6.036 Instructors]
######################################################################
...
######################################################################
# Tests [Written by 6.036 Instructors]
######################################################################
